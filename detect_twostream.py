...
def detect(opt):
    ...
    for (path, img, im0s, vid_cap), (path_, img2, im0s_, vid_cap_) in zip(dataset, dataset2):
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            '''
            annoPath = "./data/datasets/M3FD/rgb/labels/test/"
            annoName = (str(p.stem)) + ".txt"  
            annoPath += annoName
            if Path(annoPath).exists():
                 gt = np.loadtxt(annoPath)
                 if gt.size > 0:
                     gt = gt.reshape((-1, 5))
                     # gt format: [class, x_center, y_center, w, h] (normalized to original image)
                     # convert to [x1, y1, x2, y2] (absolute in original image)
                     gt_boxes = gt[:, 1:5].copy()  # [x_center, y_center, w, h]
                     gt_boxes = xywh2xyxy(gt_boxes)  # convert to [x1, y1, x2, y2] (normalized)
                     # scale to original image size
                     h0, w0 = im0.shape[:2]  # original image size
                     gt_boxes[:, [0, 2]] *= w0  # x * width
                     gt_boxes[:, [1, 3]] *= h0  # y * height
                     gt_conf = np.ones((gt.shape[0], 1))  # confidence
                     gt_cls = gt[:, 0:1]  # class
                     gt = np.hstack((gt_boxes, gt_conf, gt_cls))
                     gt = torch.from_numpy(gt).to(device)
                     gt.is_gt = True  # mark as ground truth
                     det = gt
            else:
                 print(f"GT file not found: {annoPath}")
            '''
            

            # print(det)
            if len(det):
                # For GT, coordinates are already scaled to original image size
                # For model predictions, rescale from img_size to im0 size
                # Since we set det.is_gt = True for GT, skip scaling
                if not (hasattr(det, 'is_gt') and det.is_gt):
...
if __name__ == '__main__':
    ...
    parser.add_argument('--weights', nargs='+', type=str, default="./runs/train/DroneVehicle/weights/best.pt", help='model.pt path(s)')
    parser.add_argument('--source1', type=str, default='./data/datasets/DroneVehicle/rgb/images/test', help='source')  # file/folder, 0 for webcam
    parser.add_argument('--source2', type=str, default='./data/datasets/DroneVehicle/ir/images/test', help='source')  # file/folder, 0 for webcam
