...
import cv2
#coding=utf-8s
#names = ['car','truck','people','bus','lamp','motorcycle']  # class names
target_layers = ['model_42', 'model_45', 'model_32', 'model_33', 'model_34','model_48']
#names = ['person']  # class names
names = ['car','truck','bus','van','freight_car']
#names = ['person','car','bus','motorcycle','lamp','truck']  # class names 
#target_layers = ['model_32','model_33','model_34']

# Arguments
parser = argparse.ArgumentParser()
#parser.add_argument('--model-path', type=str, default="/data/code/yolov5-modify/runs/train/exp22/weights/best.pt", help='Path to the model')
parser.add_argument('--model-path', type=str, default="./runs/train/DroneVehicle/weights/best.pt", help='Path to the model')  #cft results
#parser.add_argument('--model-path', type=str, default="/data/code/yolov5-test/mamba.pt", help='Path to the model')  #mamba results
parser.add_argument('--img-path', type=str, default="./data/datasets/DroneVehicle/rgb/images/test/07486.jpg", help='input image path')
parser.add_argument('--img-path2', type=str, default="./data/datasets/DroneVehicle/ir/images/test/07486.jpg", help='input image path')
# parser.add_argument('--img-path', type=str, default="/data/datasets/M3FD/RGB/train/images/00000.png", help='input image path')
# parser.add_argument('--img-path2', type=str, default="/data/datasets/M3FD/ir/train/images/00000.png", help='input image path')
parser.add_argument('--output-dir', type=str, default='runs/dronevehicle', help='output dir')
...
def main(img_path):
    ...
    for target_layer in target_layers:
        ...
        masks, logits, [boxes, _, class_names, conf] = saliency_method(torch_img,torch_img2)  
        ...
        for i, mask in enumerate(masks):
            ...
            res_img = plot_one_box(bbox, res_img, label=None, color=colors[int(names.index(cls_name))],
                                   line_thickness=3)
