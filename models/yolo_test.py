...
class Detect(nn.Module):
    ...
    def forward(self, x):
        ...
        for i in range(self.nl):
            ...
            if not self.training:  # inference
                ...
                y = torch.sigmoid(x_i)  
                y_xy = (y[..., 0:2] * 2. - 0.5 + self.grid[i].to(y.device)) * self.stride[i].to(y.device)  # xy
                y_wh = (y[..., 2:4] * 2) ** 2 * self.anchor_grid[i]  # wh
                y = torch.cat([y_xy, y_wh, y[..., 4:]], dim=-1)
        ...
        if self.training:
            ...
            return (torch.cat(z, 1), x_processed)
    ...
    '''def _make_grid(self, nx=20, ny=20, i=0):
        ...
        yv, xv = torch.meshgrid(y, x, indexing="ij") if False else torch.meshgrid(y, x)  # torch>=0.7 compatibility
    ...
    def forward(self, x, x2, augment=False, profile=False):
        if augment:
            ...
            for si, fi in zip(s, f):
                ...
                yi = self.forward_once(xi, x2, profile)  # forward
...
def parse_model(d, ch):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, module, args
        ...
        if m in [Conv, GhostConv, Bottleneck, Bottleneck_LA, GhostBottleneck, SPP, DWConv, MixConv2d, Focus, CrossConv, BottleneckCSP,
                 C3,C2f, InitConv,InitConv1,C3TR,SPPF,C2f_LA]:
            if m is InitConv:
                c1, c2 = 3, args[0]
                ...
                args = [c1, c2, *args[1:]]
                if m in [BottleneckCSP, C3,C2f, C2f_LA, C3TR]:
            ...
            args = [c2, args[1]]
        elif m in (VSSLayer,PatchMerging2D,VSSLayer_up,PatchExpand2D):
            c2=args[0]
            if c2 != no:  # if not output
               c2 = make_divisible(c2 * gw, 8)  
            args = [c2,*args[1:]]
        elif m in (chnnel_swapping_Vmamba,CrossLayer,dual_swapping_Vmamba,OSA,SID):
            c2=args[0]
            if c2 != no:  # if not output
               c2 = make_divisible(c2 * gw, 8)  
            args = [c2,*args[1:]]
            #args = [*args[:]]
        elif m is PatchEmbed2D:
            t=args[2]
            if t != no:  # if not output
               t = make_divisible(t * gw, 8) 
            args=[args[0],args[1],t]
        elif m is ASFF2:
            c1, c2 = [ch[f[0]], ch[f[1]]], args[0]
            c2 = make_divisible(min(c2, 1024) * gw, 8)
            args = [c1, c2, *args[1:]]
        elif m is ASFF3:
            #print(f)
            #print(ch)
            ...
            c2 = 2 * ch[f]  # WTDM doubles the number of channels
        elif m is DSDM:
            c2 = 2 * ch[f]  # DSDM doubles the number of channels through PatchMerging2D_new
        else:
            c2 = ch[f]
