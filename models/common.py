...
try:
    ...
    FUSED = False
    print("natten 0.17 or higher is not installed, falling back to non-fused implementation")
'''
...
class Upsample(nn.Module):
     def __init__(self, c1, c2, scale_factor=2):
         ...
         if scale_factor == 2:
             ...
             self.cv1 = nn.ConvTranspose2d(c1, c2, 1, 1, 0, bias=True)
     def forward(self, x):
         c1 = self.c1
         c2 = self.c2
         return self.cv1(x)
class ASFF2(nn.Module):
     ...
     def forward(self, x):
         ...
         #print(x_level_0.size(),x_level_1.size())
         if self.level == 0:
         ...
         #print(level_0_resized.size(),level_1_resized.size())
         level_0_weight_v = self.weight_level_0(level_0_resized)
         level_1_weight_v = self.weight_level_1(level_1_resized)
        # print(level_0_weight_v.size(),level_1_weight_v.size())
         ...
         #print(self.conv(fused_out_reduced).size())
...
class ASFF3(nn.Module):
    def __init__(self, c1, c2, level=0):
        ...
        if level == 1:
            self.stride_level_0 = Conv(c1_l, self.inter_dim, 2, 2, 0)
            self.stride_level_2 = Upsample(c1_h, self.inter_dim, scale_factor=2)
        if level == 2:
            self.stride_level_0 = Conv(c1_l, self.inter_dim, 4, 4, 0)
            self.stride_level_1 = Conv(c1_m, self.inter_dim, 2, 2, 0)
            self.stride_level_2 = Conv(c1_h, self.inter_dim, 1, 1, 0)
        self.weight_level_0 = Conv(self.inter_dim, compress_c, 1, 1)
        self.weight_level_1 = Conv(self.inter_dim, compress_c, 1, 1)
...
class SPPF(nn.Module):
    ...
    def forward(self, x):
        ...
        y1 = self.m(x)
        y2 = self.m(y1)
        #print(self.cv2(torch.cat((x, y1, y2, self.m(y2)), 1)).shape)
...
class PatchMerging2D_new(nn.Module):
    r""" Patch Merging Layer.
    Args:
        input_resolution (tuple[int]): Resolution of input feature.
        dim (int): Number of input channels.
...
class BiAttn(nn.Module):
    def __init__(self, in_channels, act_ratio=0.125, act_fn=nn.GELU, gate_fn=nn.Sigmoid):
        super().__init__()
        self.reduce_channels = int(in_channels * act_ratio)
        self.norm = nn.LayerNorm(in_channels)
        self.global_reduce = nn.Linear(in_channels, self.reduce_channels)
        self.channel_select = nn.Linear(self.reduce_channels, in_channels)
        self.local_reduce = nn.Linear(in_channels, self.reduce_channels)
        self.spatial_select = nn.Linear(self.reduce_channels * 2, 1)
        self.act_fn = act_fn()  
        self.gate_fn = gate_fn()
    ...
    def forward(self, x):
        ...
        x_global = x.mean(dim=[1, 2], keepdim=True)

        x_global = self.act_fn(self.global_reduce(x_global))
        c_attn = self.gate_fn(self.channel_select(x_global))
        
        x_local = self.act_fn(self.local_reduce(x))
        x_global_broadcast = x_global.repeat(1, x.shape[1], x.shape[2], 1)
        x_fuse = torch.cat([x_local, x_global_broadcast], dim=-1)
        s_attn = self.gate_fn(self.spatial_select(x_fuse))
        
        bi_attn = c_attn * s_attn
        bi_attn = bi_attn.permute(0, 3, 1, 2)
...
class WTDM(nn.Module):
    def __init__(self, in_channels):
        ...
        self.BA = BiAttn(in_channels=in_channels)
        ...
        self.gate = nn.Sequential(
            nn.Conv2d(3*in_channels, in_channels, kernel_size=1, stride=1,bias = True),
            nn.SiLU(inplace=True)
        )
        self.Conv_gate = nn.Conv2d(3*in_channels, in_channels, kernel_size=1, stride=1, bias=True)
    ...
    def create_wavelet_filter(self, wave, in_size, dtype=None):
        w = pywt.Wavelet(wave)
    ...
    def forward(self, x):
        ...
        dec_filters = self.create_wavelet_filter(wave='haar', in_size=self.in_channels, dtype=x.dtype) 
        x_wave = self.wavelet_transform(x, dec_filters)
        A, H_high, V, D = x_wave[:, :, 0], x_wave[:, :, 1], x_wave[:, :, 2], x_wave[:, :, 3]

        F_H = torch.cat([H_high, V, D], dim=1)   
        F_H = self.avgpool(F_H)

        F_H_gate = self.gate(F_H)
        F_H_ = self.Conv_gate(F_H)
        F_H = F_H_gate * F_H_

        x_ba = self.BA(A)
        A = A * x_ba

        wave = torch.cat([A,F_H], dim=1)
        fused = self.out(wave)
...
class dual_Vmamba(nn.Module):
    ...
    def forward(self, x1, x2): 
        ...
        #y = self.out_proj(z.permute(0, 2, 3, 1)).permute(0, 3, 1, 2) 

        #x_1, x_2 = torch.chunk(y, chunks=2, dim=1)  
...
class dual_swapping_Vmamba(nn.Module): 
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.out_proj1 = SE(dim)
        self.out_proj2 = SE(dim)
    ...
    def forward(self, x):
        ...
        #x1_swap[:,even_idx], x2_swap[:,even_idx] = x2[:,even_idx].type_as(x1_swap), x1[:,even_idx].type_as(x2_swap)
        ...
        x_1 = self.out_proj1(x1.permute(0, 2, 3, 1)).permute(0, 3, 1, 2)
        x_2 = self.out_proj2(x2.permute(0, 2, 3, 1)).permute(0, 3, 1, 2)
...
class CB(nn.Module):
    def __init__(self, num_feat):
        super().__init__()
        #self.out_norm = nn.LayerNorm(num_feat)
    ...
    def forward(self, x):  
        ...
        y1 = self.act1(avg_out)
        y2 = self.act2(max_out)
        z1 = x1 * y1
        z2 = x2 * y2

        z1_unsq = z1.unsqueeze(2)  
        z2_unsq = z2.unsqueeze(2)  
        z = torch.cat([z1_unsq, z2_unsq], dim=2).flatten(1, 2)  #Channel-wise alternate concatenation
...
class SID(nn.Module):
    ...
    def forward(self, x):   
        rgb_feat = x[0]
        ir_feat = x[1]
        return self.extractor(rgb_feat), self.extractor(ir_feat)
...
class DeformableNeighborhoodAttention(nn.Module):
    ...
    @staticmethod
    def na2d_qk(q, k, kernel_size, dilation=1, rpb=None):
        B, G, H, W, Cg = q.shape
        ks = kernel_size[0]
        pad = (ks // 2) * dilation
        q = einops.rearrange(q, 'b g h w c -> (b g) c h w')
        k = einops.rearrange(k, 'b g h w c -> (b g) c h w')
        k = F.unfold(k, kernel_size=ks, dilation=dilation, padding=pad, stride=1)
        k = einops.rearrange(k, '(b g) (c k) (h w) -> b g h w c k', g=G, c=Cg, k=ks**2, h=H, w=W)
        q = einops.rearrange(q, '(b g) c h w -> b g h w c 1', g=G)
        attn = (q * k).sum(-2)
        if rpb is not None:
            rpb = einops.rearrange(rpb, 'h k1 k2 -> h (k1 k2)')
            attn += rpb[None, :, None, None, :]
        return attn

    @staticmethod
    def na2d_av(attn, v, kernel_size, dilation=1):
        B, G, H, W, K = attn.shape
        Cg = v.shape[-1]
        ks = kernel_size[0]
        pad = (ks // 2) * dilation
        v = einops.rearrange(v, 'b g h w c -> (b g) c h w')
        v = F.unfold(v, kernel_size=ks, dilation=dilation, padding=pad, stride=1)
        v = einops.rearrange(v, '(b g) (c k) (h w) -> b g h w k c', g=G, c=Cg, k=ks**2, h=H, w=W)
        attn = einops.rearrange(attn, 'b g h w k -> b g h w k 1')
        out = (attn * v).sum(-2)
        return out
    # ---------------------------------------------------------------------------------
    ...
    def forward(self, x, external_offset):
        B, C, H, W = x.size()
        dtype, device = x.dtype, x.device

        assert C == self.nc, f"Input channel {C} must match module dim {self.nc}"
        #assert external_offset.shape == (B, 16, H, W), \
            #f"external_offset shape must be [B,16,H,W], got {external_offset.shape}"
        #assert external_mask.shape == (B, 8, H, W), \
        #    f"external_mask shape must be [B,8,H,W], got {external_mask.shape}"

        q = self.proj_q(x)

        offset = einops.rearrange(
            external_offset,
            'b (p g) h w -> (b g) h w p',
            p=2, g=self.n_groups
        )
        ...
        #x_sampled = x_sampled * ext_mask.permute(0, 3, 1, 2)
...
class OSA(nn.Module):
    ...
    def __init__(self, hidden_dim):
        ...
        self.offset_pred = nn.Sequential(
            nn.Conv2d(hidden_dim*2, hidden_dim, 3, 1, 1, bias=False),
            nn.ReLU(),
            nn.Conv2d(hidden_dim, 2*8, 3, 1, 1)  # 2×K（K=8）
        )

        self.dna = DeformableNeighborhoodAttention(
            dim=hidden_dim, num_heads=8, kernel_size=3, dilation=1, rel_pos_bias=False
        )
        '''
    ...
    def forward(self, x):
        ...
        #M = self.ir_attention(ir_feat)  
    
        #feat_cat = torch.cat([ir_feat, rgb_feat], dim=1)  # (B,2C,H,W)
        #Mw = M * feat_cat  

        #offset_mask = self.offset_pred(Mw)  

        
        #offset = offset_mask[:, :2*8, :, :]  
    
        
        #aligned_rgb = self.dna(rgb_feat, offset)
        aligned_rgb = rgb_feat

        return aligned_rgb, ir_feat
...
if __name__ == "__main__":
    net = dual_swapping_Vmamba(dim=128,depth=3).cuda()
    x1 = torch.rand((1,64,320, 320)).cuda()
    x2 = torch.rand((1,64,320, 320)).cuda()
    x1 = DSDM(dim=64,depth=1).cuda()(x1)
    x2 = WTDM(in_channels=64).cuda()(x2)    
    x = (x1, x2)
    x1, x2 = net(x)
    print(x1.shape)
    print(x2.shape)