...
class YOLOV5TorchObjectDetector(nn.Module):
    def __init__(self,
        ...
        self.model = attempt_load(model_weight)
    ...
    def forward(self, img,img2):
        output = self.model(img, img2, augment=False)
        prediction = output
        while isinstance(prediction, tuple):
        ...
        # Return format for GradCAM compatibility
        # preds: [boxes, classes, class_names, confidences]
        # logits: raw logits from the model output
